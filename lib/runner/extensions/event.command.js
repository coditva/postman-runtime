var _ = require('lodash'),
    uuid = require('uuid'),
    async = require('async'),
    tjs = require('teleport-javascript'),
...
module.exports = {
    init: function (done) {
        ...
        sandbox.createContext(_.merge({
            ...
            context.on('console', function (cursor, level) {
                let args = Array.prototype.slice.call(arguments, 2);

                args = tjs.parse(args);
                args.unshift(cursor, level);
                run.triggers.console.apply(run.triggers, args);
            });
